...
export class IngestionController {
  private documentParser = new DocumentParserService();
  private parserAgent = new ParserAgentService();
   ...
   */
  async extractTestCases(req: Request, res: Response): Promise<void> {
    try {
      ...
      const extraction = await this.parserAgent.extractTestCases(
        parsed.text,
        documentId,
        { module: module as string, priority: priority as string }
      );

      // Store test cases
      const savedTestCases = await Promise.all(
        extraction.testCases.map(tc => this.testCaseRepo.create(tc))
      );
  ...
  private determineSourceType(mimetype: string, filename: string): 'pdf' | 'word' | 'excel' | 'html' | 'markdown' | 'text' {
