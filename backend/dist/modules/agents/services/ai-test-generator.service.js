...
export class AiTestGeneratorService {
    ...
    executorUrl = process.env.EXECUTOR_URL || 'http://localhost:8001';
    /**
    ...
    async generateTests(input) {
        ...
        const response = await this.router.chat([
            {
                role: 'system',
                content: 'You are a senior QA engineer and test automation expert. ' +
    ...
    // Private helpers
    // ------------------------------------------------------------------
    async captureDomSnapshot(url) {
        try {
            const res = await axios.post(`${this.executorUrl}/snapshot`, { url }, { timeout: 30000 });
            return res.data?.dom ?? '';
        }
        catch (err) {
    ...
    buildPrompt(input, domSnapshot, count) {
        ...
        const moduleFocus = input.modules?.length
            ...
            : '';
        return (`Generate ${count} Playwright-ready test cases for the following scenario:\n\n` +
            `URL: ${input.targetUrl}\n` +
...
//# sourceMappingURL=ai-test-generator.service.js.map