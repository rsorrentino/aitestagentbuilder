...
export function authorize(...allowedRoles) {
    return (req, res, next) => {
        ...
        if (allowedRoles.includes(role) || permissions.includes('*')) {
            return next();
        }
...
async function validateApiKey(apiKey) {
    try {
        ...
        const result = await db.query(`SELECT id, user_id, permissions, expires_at 
       FROM api_keys 
       WHERE key_hash = $1 AND active = true 
       AND (expires_at IS NULL OR expires_at > NOW())`, [hashedKey]);
